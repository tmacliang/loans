...
import org.kelex.loans.bean.RefundDTO;
...
public class RefundService extends TransactionService<RefundRequest> {
    ...
    protected void checkBusinessLogic(TransactionRequestContext<? extends RefundRequest> context) throws Exception {
        RequestDTO<? extends RefundRequest> request = context.getRequest();
        RefundDTO data = (RefundDTO) request.getData();
        ...
        CurrProcessCtrlEntity currProcessCtrl = entryService.findOneCurrProcessCtrl(data.getCurrencyCode(), planProcessCtrl.getCurrencyUnit(), context.getRepository());
        BigDecimal refundAmount = data.getRefundAmount().multiply(BigDecimal.ONE.scaleByPowerOfTen(currProcessCtrl.getPower()));
...
//            context.getTxnSummaries().addAll(interestTxnList);
    ...
    public TxnSummaryEntity createRefundAmtTxn(TransactionRequestContext<? extends RefundRequest> context) {
        ...
        TxnProfileEntity reverseTxnProfile = entryService.findOneTxnProfile(reverseCode, repository);
        ...
        refundTxn.setId(id);
        refundTxn.setActTypeId(account.getActTypeId());
        ...
        refundTxn.setTxnType(reverseTxnProfile.getTxnType());
        ...
        refundTxn.setFlowType(reverseTxnProfile.getFlowType());
        refundTxn.setTxnDate(request.getBusinessDate());
        refundTxn.setTxnTime(LocalTime.now());
        refundTxn.setOriginalTxnId(0L);
        refundTxn.setReversalAmt(ZERO);
    ...
    public TxnSummaryEntity createRefundFeeTxn(TransactionRequestContext<? extends RefundRequest> context) {
        ...
        refundFeeTxn.setActTypeId(account.getActTypeId());
        ...
        refundFeeTxn.setOriginalTxnId(0L);
        refundFeeTxn.setReversalAmt(ZERO);
